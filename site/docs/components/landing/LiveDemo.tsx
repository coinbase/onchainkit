...
import CopyIcon from '../svg/CopySvg.tsx';  // Adjust the import path as necessary
...
function highlightJSX(code: string): React.ReactNode {
  ...
  return lines.map((line, lineIndex) => (
    <React.Fragment key={lineIndex}>
      {line.split(/(<.+?>|{.+?}|\s+)/).filter(Boolean).map((part, partIndex) => {
        if (part.trim().startsWith('//')) {
          return <span key={partIndex} className="text-green-500 dark:text-green-400">{part}</span>;
        } else if (part.trim().startsWith('import')) {
          return <span key={partIndex} className="text-purple-500 dark:text-purple-400">{part}</span>;
          ...
          return (
            <span key={partIndex}>
              ...
              <span className={
                customComponents.has(tagName) ? "text-yellow-500 dark:text-yellow-400" :
                tagName[0] === tagName[0].toUpperCase() ? "text-teal-500 dark:text-teal-400" : 
                  ...
                  <span key={attrIndex}>
                    ...
                    {attrValue && (
                      <>
                        ...
                        <span className="text-orange-500 dark:text-orange-400">{attrValue}</span>
                      </>
                    )}
                  </span>
                );
          ...
          return <span key={partIndex} className="text-cyan-500 dark:text-cyan-400">{part}</span>;
        } else if (customComponents.has(part)) {
          return <span key={partIndex} className="text-yellow-500 dark:text-yellow-400">{part}</span>;
        } else if (/^[A-Z][a-zA-Z]*$/.test(part)) {
          return <span key={partIndex} className="text-teal-500 dark:text-teal-400">{part}</span>;
        } else if (/^[a-z][a-zA-Z]*$/.test(part)) {
          return <span key={partIndex} className="text-cyan-500 dark:text-cyan-400">{part}</span>;
        }
        ...
        <div className="w-full lg:w-[800px] lg:flex-shrink-0 border-[1px] rounded-lg border-zinc-200 dark:border-zinc-800">
          <div className="w-full bg-zinc-100 dark:bg-zinc-900 rounded-lg relative h-[400px] lg:h-[600px] overflow-hidden">
            <div className="flex border-b border-zinc-200 dark:border-zinc-700">
              ...
              <button
                className={`py-2 px-4 mt-2 text-sm ${
                  activeSubTab === 'code'
                    ? 'bg-zinc-100 dark:bg-zinc-800 text-zinc-950 dark:text-zinc-50 border-b-2 border-indigo-600 dark:border-indigo-400'
                    : 'text-zinc-700 dark:text-zinc-300'
                }`}
            ...
            <div className="h-[calc(100%-40px)] overflow-auto">
              <div className={`${activeSubTab === 'preview' ? 'flex' : 'hidden'} p-4 h-full items-center justify-center`}>
                <div className="w-full h-full flex items-center justify-center overflow-auto">
                  <ActiveComponent />
                </div>
              </div>
              <div className={`${activeSubTab === 'code' ? 'block' : 'hidden'} h-full`}>
                <button
