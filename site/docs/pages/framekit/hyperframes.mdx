...
Next, in `api/route.ts`, or another file and imported there, add this frame to the `frames` array:
...
async function getResponse(req: NextRequest): Promise<NextResponse> {
  ...
  if (!frame) { // [!code focus]
    return new NextResponse('Frame name not provided in postUrl', { status: 404 }); // [!code focus]
  } // [!code focus]
  ...
  return new NextResponse(getHyperFrame(frame as string, text || '', message?.button)); // [!code focus]
}
```
