...
async function getResponse(req: NextRequest): Promise<NextResponse> {
  ...
  if (!frame) { // [!code focus]
    return new NextResponse('Frame name not provided in postUrl', { // [!code focus]
      status: 404 // [!code focus]
    }); // [!code focus]
  } // [!code focus]
  ...
  return new NextResponse( // [!code focus]
    getHyperFrame(frame as string, text || '', message?.button) // [!code focus]
  ); // [!code focus]
}
```
