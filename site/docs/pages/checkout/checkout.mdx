...
To customize the name and logo of your application rendered in keys.coinbase.com, set the `name` and `logo` values inside [OnchainKitProvider](/config/onchainkit-provider#usage).
...
<img alt="Customizing the name and logo"
...
You can override component styles using `className`. You can also customize your [OnchainKit theme](/guides/themes).
```tsx twoslash
