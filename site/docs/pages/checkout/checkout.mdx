...
For the `Checkout` component, you'll need to provide an API Key. You can get one following our [Getting Started](/getting-started#get-your-public-api-key) steps.

```bash
npm create onchain@latest
```

If you're adding the component to an existing project, simply install OnchainKit (`npm install @coinbase/onchainkit@latest`). Wrap the `<OnchainKitProvider />` around your app, following the steps in [Getting Started](/getting-started#add-providers).
...
For non-idempotent actions, like rendering your own success screen, you can simply check that the `statusName` is equal to `success`.

For idempotent actions, like order fulfillment, we recommend one of the following approaches to verify a charge has been fully paid.
...
You can customize the text using the `text` prop on `CheckoutButton`.
```tsx twoslash
