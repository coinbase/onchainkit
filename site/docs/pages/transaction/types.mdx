...
type TransactionReact = {
  calls?: Call[] | Promise<Call[]>; // An array or Promise that resolves to an array of calls to be made in the transaction. Mutually exclusive with the `contracts` prop.
  ...
  contracts?: ContractFunctionParameters[] | Promise<ContractFunctionParameters[]>; // An array or Promise that resolves to an array of contract function parameters for the transaction. Mutually exclusive with the `calls` prop.
