...
```tsx twoslash
// @noErrors: 2580 2304 2451 2365 2362 1005 1109 2552 - Cannot find name 'process', Cannot find name 'contracts'
...
Execute one or multiple transactions using the Transaction component. You can pass transactions in either `Call` or `ContractFunctionParameters` format. 

<br/>

Using `Call` format instead of `ContractFunctionParameters` is a great way to bundle several operations into a single transaction for smart wallets. Externally Owned Accounts (EOAs) such as the Coinbase Wallet extension will execute each call one-at-a-time with a separate pop-up for each.

<br/>

A transaction in `ContractFunctionParameters` format is defined as:
...
type ContractFunctionParameters = {
  ...
  functionName: ContractFunctionName; // a function to extract from the ABI
  args: ContractFunctionArgs; // array of arguments to pass to the function call
};
```

<br/>

A transaction in `Call` format is defined as:

```tsx twoslash
// @noErrors: 2304 Cannot find name 'Hex'
type Call = {
  to: Hex; // the address you are executing the call on
  data?: Hex; // the encoded function data to pass along in the call (optional)
  value?: bigint; // the amount of ether to send along with the call, denominated in wei (optional)
};
```
