...
The `<WalletAdvanced />` components provide an advanced Wallet interface for users,
including:
...
Designed for desktop experiences, the `WalletAdvanced` component gives
...
You will see two Wallet buttons below. Right below us is `WalletAdvancedDefault`.
...
Make sure to update the `appName` as that will be displayed to the user when
...
### Drop in the `<Wallet />` and `<WalletAdvanced />` components
