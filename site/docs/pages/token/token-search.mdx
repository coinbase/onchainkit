import { TokenSearch } from '../../../../src/token/components/TokenSearch';
import App from '../App';
...
The `<TokenSearch />` component allows users to search on a given list of tokens by name, symbol or address with an optional debounce delay.
...
<TokenSearch tokens={tokens} onSearch={(tokens) => {}} />; // [!code focus]
```
...
<App>
  <TokenSearch onSearch={(tokens) => console.log('Found tokens:', tokens)} tokens={tokens} />
</App>
