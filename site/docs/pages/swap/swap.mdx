...
## Coinbase Smart Wallet popup tips 
When using Coinbase Smart Wallets, be aware of the potential browser pop-ups that may block your app from interacting with the wallet.

We recommend following [this guide](https://www.smartwallet.dev/guides/tips/popup-tips) to improve the user experience.

If the Smart Wallet popup opens and displays an error or infinite spinner, it may be due to the app's `Cross-Origin-Opener-Policy`. Be sure to use a directive that allows the Smart Wallet popup to function.
