...
describe('NftProvider', () => {
  it('should provide NFT data to its children', () => {
    ...
    expect(getByAltText('Test NFT')).toHaveAttribute('src', 'http://example.com/test-nft.png');
  });
  ...
  it('should use a custom useNftData hook if provided', () => {
    ...
    const { getByText, getByAltText } = render(
      <NftProvider contractAddress="0x123" tokenId="1" useNftData={customUseNftData}>
        <MockComponent />
      </NftProvider>
    );
    ...
    expect(getByAltText('Custom NFT')).toHaveAttribute('src', 'http://example.com/custom-nft.png');
  });
});
